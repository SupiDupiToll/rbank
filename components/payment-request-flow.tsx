...
export function PaymentRequestFlow({ payerUserId, recipientEmail }: PaymentRequestFlowProps) {
  ...
  function handleDigitInput(digit: string) {
    setMessage("");
    setPin((currentPin) => (currentPin.length >= PIN_LENGTH ? currentPin : `${currentPin}${digit}`));
  }
    ...
    <div className="mx-auto max-w-2xl">
      {step === "amount" ? (
        <Card className="space-y-8">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.3em] text-primary">Schritt 1</p>
            <h1 className="mt-3 text-3xl font-display text-slate-100">Betrag eingeben</h1>
            <p className="mt-3 text-slate-300">Lege zuerst fest, wie viel an {recipientEmail} gesendet werden soll.</p>
          </div>
          ...
          <Button className="w-full rounded-xl" onClick={goToPinStep} type="button">
            Weiter zur PIN
          </Button>
        </Card>
      ) : (
        <Card className="space-y-8">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-display text-slate-100">PIN eingeben</h2>
              <p className="mt-3 text-slate-300">Bestaetige die Zahlung mit der 4-stelligen PIN des zahlenden Nutzers.</p>
            </div>
            <Button onClick={() => setStep("amount")} type="button" variant="outline">
              Zurück
            </Button>
          </div>
          ...
          <div className="rounded-3xl border border-primary/20 bg-primary/5 p-5">
            ...
            <p className="mt-2 text-4xl font-display text-primary">{formatEuroFromCents(amountCents)}</p>
            <p className="mt-4 text-sm text-slate-400">Empfänger</p>
            <p className="mt-1 font-semibold text-slate-100">{recipientEmail}</p>
          </div>

          <div className="space-y-3">
            <p className="text-sm font-semibold text-slate-200">PIN des zahlenden Nutzers</p>
              ...
              {Array.from({ length: PIN_LENGTH }, (_, index) => (
                <div
                  ...
                  className={cn(
                    ...
                    index < pin.length ? "border-primary/40 bg-primary/10 text-primary" : "border-slate-800 bg-slate-900/70 text-slate-500"
                  )}
                >
                  {index < pin.length ? "*" : ""}
                </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <Button className="h-16 rounded-2xl text-lg" disabled={isSubmitting} onClick={handleBackspace} type="button" variant="outline">
              Löschen
            </Button>
            <Button
              className="h-16 rounded-2xl text-2xl"
          ...
          <Button className="w-full rounded-xl" disabled={isSubmitting || pin.length !== PIN_LENGTH} onClick={handleConfirm} type="button">
