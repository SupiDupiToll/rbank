...
import { deferPayment } from "@/lib/loan";
...
export async function POST(request: Request, context: Params) {
  return safeRoute(async () => {
    ...
    try {
      const result = await deferPayment(loanId, user.id);
      ...
      if (err instanceof Error) {
        ...
        if (err.message === "NO_PAYMENT_TO_DEFER") {
          return NextResponse.json({ error: "Keine Rate zum Aussetzen." }, { status: 400 });
        }
      }
      throw err;
    }
