...
export async function POST(request: Request) {
  return safeRoute(async () => {
    ...
    const rateLimitError = await enforceRateLimit(request, rateLimitPolicies.customerTransfer, recipient.id);
    ...
    if (body.payerUserId === recipient.id) {
      return NextResponse.json({ error: "Zahlungen vom eigenen Konto sind nicht erlaubt." }, { status: 400 });
    }
    ...
    const transferResult = await prisma.$transaction(
      ...
      if (caughtError instanceof Error && caughtError.message === "INVALID_PIN") {
        return "INVALID_PIN" as const;
      }

      if (caughtError instanceof Error && caughtError.message === "PAYMENT_REJECTED") {
        return null;
      }
    ...
    if (!transferResult) {
      return NextResponse.json({ error: "Zahlung konnte nicht ausgefuehrt werden." }, { status: 400 });
    }
